...
/// <summary>
/// Password-reset email. Same paper style as OrderEmailTemplate, driven by
/// EmailAppearanceOptions (theme) + EmailTemplateCopy (per-type/language copy).
/// The reset link carries the Identity token; expiry is surfaced in the copy.
/// </summary>
public static class ResetPasswordEmailTemplate
{
    public static string Build(EmailAppearanceOptions look, EmailTemplateCopy copy, string resetLink, int expiryMinutes, string? displayName = null)
    {
        ...
        var expiryText = isArabic
            ? $"هذا الرابط صالح لمدة {expiryMinutes} دقيقة ولمرة واحدة فقط."
            : $"This link is valid for {expiryMinutes} minutes and can be used once.";
